...
可以演进成类似 Redis Cluster 的多 shard 集群，但它不是把现有 1P1S 简单多起几个进程就完成。需要补齐：
...
推荐路径是先把“多 shard 静态路由”做成生产可用，再做在线迁移和自动 rebalance。
...
| External `1P1S` binary + `GRPC` replication committed         | 高频主入口 + 单备闭环真实 committed         | `140,246` | `140,246` | `123,799` |
| External `1P1S` binary + `AERON` replication committed        | 高频主入口 + 单备闭环真实 committed         | `78,915` | `78,915` | `71,822` |
| External `1P2S` binary + `GRPC` quorum replication committed  | 高频主入口 + 两备 quorum 闭环真实 committed | `85,662` | `85,662` | `40,707` |
| External `1P2S` binary + `AERON` quorum replication committed | 高频主入口 + 两备 quorum 闭环真实 committed | `126,513` | `126,513` | `40,952` |
| External `1P3S` binary + `GRPC` quorum replication committed  | 高频主入口 + 三备 quorum 闭环真实 committed | `90,155` | `90,155` | `54,689` |
| External `1P3S` binary + `AERON` quorum replication committed | 高频主入口 + 三备 quorum 闭环真实 committed | `120,964` | `120,964` | `19,464` |
