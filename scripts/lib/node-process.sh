...
start_matcher_node() {
  ...
  local zk_connect="${ZK_CONNECT:-127.0.0.1:2181}"
  ...
  local data_dir="$data_root/$node_id"
  ...
  local cmd=(
    ...
    -Dmatcher.httpBindHost=127.0.0.1
    -Dmatcher.grpcPort="$grpc_port"
    -Dmatcher.grpcBindHost=127.0.0.1
  ...
  if [[ -n "$binary_port" ]]; then
    cmd+=(
      ...
      -Dmatcher.binaryIngressBindHost=127.0.0.1
    )
  fi
